...
export async function loadCredentials(): Promise<CredentialsFile | null> {
  ...
  // [TokenSea] Environment variable source — highest priority, injected
  // per-container for stateless deployment. If any DARIO_OAUTH_* env var
  // is present, construct a CredentialsFile and enter it into the
  // freshest-credential competition.
  const envAccessToken = process.env.DARIO_OAUTH_ACCESS_TOKEN;
  const envRefreshToken = process.env.DARIO_OAUTH_REFRESH_TOKEN;
  if (envAccessToken && envRefreshToken) {
    const envExpiresAt = process.env.DARIO_OAUTH_EXPIRES_AT
      ? Number(process.env.DARIO_OAUTH_EXPIRES_AT)
      : 0;
    const envScopes = process.env.DARIO_OAUTH_SCOPES
      ? process.env.DARIO_OAUTH_SCOPES.split(',').map(s => s.trim())
      : ['user:inference'];
    candidates.push({
      claudeAiOauth: {
