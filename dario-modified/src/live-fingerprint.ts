...
export function loadTemplate(_options?: { silent?: boolean }): TemplateData {
  // [TokenSea] Support external template file via environment variable.
  // In containers without CC CLI, operators can pre-capture a template
  // on a machine with CC installed and mount it into the container.
  const envTemplatePath = process.env.DARIO_TEMPLATE_PATH;
  if (envTemplatePath) {
    try {
      const raw = readFileSync(envTemplatePath, 'utf-8');
      return JSON.parse(raw) as TemplateData;
    } catch (err) {
      console.error(`[dario] DARIO_TEMPLATE_PATH (${envTemplatePath}) read failed: ${(err as Error).message}, falling back`);
    }
  }
...
export async function refreshLiveFingerprintAsync(options?: {
  ...
  if (!findClaudeBinary()) {
    // [TokenSea] In containers without CC binary, skip capture silently
    // when DARIO_NO_LIVE_CAPTURE=1 or when no binary is found. Return
    // null so the caller falls back to bundled or env-specified template.
    if (process.env.DARIO_NO_LIVE_CAPTURE === '1' || process.env.DARIO_TEMPLATE_PATH) {
      return null;
    }
    return null;
  }
